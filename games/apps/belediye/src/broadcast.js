...
export function tvPool(list, v, max, mode) {
  ...
  for (const e of list) {
    ...
    else if (mode === "sub" || mode === "tick") { if (s.length > max) s = tvFill(t, v.vars, true); if (mode === "sub") s = tvCap(s); }
...
export function tvDraw(pool, rng, seen) {
  ...
  for (const x of from) if ((r -= x.w) < 0) { pick = x; break; }
  seen?.add?.(pick.s);
  return pick.s;
}
...
export const TV_MAH = {
  ...
  "Lojmanlar": { de: "Lojmanlar'da", s: "Lojman", sub: [
  ...
  "Öğrenci yurdu": { de: "Öğrenci yurdunda", sub: [
  ...
  "Yukarıkavak": { de: "Yukarıkavak'ta", sub: [
...
export const TV_ONDE = {
  ...
  nermin: ["Nermin Hanım önde; tutanakları kendi defterine de ayrıca yazıyor",
...
export const TV_SOZ = {
  you: {
    win: ["Bu zafer Karakavak'ın; çaylar benden.", "Oy vermeyenlerin de başkanıyım, çay vermeyenlerin de.",
      ...
      "Rakiplerime geçmiş olsun; çaylar benden, simitler onlardan.", "Beş yıl daha evrak, beş yıl daha çay. Hayırlı olsun.",
    ...
    lose: ["Halkın kararı başımızın üstünde; semaveri yanımda götürüyorum.", "Kaybetmedik, beş yıl mola verdik.",
      "Fikret, son çayı demle; iki şekerli olsun.", "Yukarıkavak geç kaldı; traktörü affediyorum.",
      "Makamı devrediyorum; Tekir'i devretmiyorum.", "Sandık haklıdır. Ben de haklıydım ama sandık daha haklı."],
  },
  nermin: {
    win: ["Şeffaf belediye bugün başlıyor; ilk iş çay bardaklarını sayacağız.", "Tutanakları okudum, hepsini. Kazandık.",
      "Meclise taşıdık, sandıktan çıktı.", "Kasayı boş bulursak basın toplantısı yarın saat onda.",
      "Beş yıldır tutanağa yazdırıyordum; bu gece sandık yazdı.", "İlk icraat: makam odasına cam kapı. Şeffaflık kapıdan başlar.",
      "Muhalefete sesleniyorum: beni benim kadar denetleyemezsiniz.", "Çaylar artık fişli; fişi herkes görecek."],
    lose: ["Tutanağa yazdırıyorum: itirazım var.", "Meclise taşıyacağım; bu kez sandığı.", "Şeffaf kaybettik, alnımız ak."],
  },
  vekil: {
    win: ["Ankara'ya teşekkür ederim; Karakavak'a da. Önce Ankara'ya.", "Talimat geldi: kazandık.",
      "Genel merkez duydu; şimdi Karakavak da duysun.", "Ankara'yla el ele; elim biraz yoruldu ama."],
    lose: ["Ankara'ya iletirim; Ankara da bana iletir.", "Talimat gelmediği için yorum yapmıyorum.", "Sonucu yukarıya arz ettim."],
  },
  cengiz: {
    win: ["Beton geleceğimizdir; gelecek yarın sabah dökülüyor.", "Her mahalleye bir Towers; ilk temel pazartesi.",
      "Oy verenlere daire, vermeyenlere manzara.", "Makam odası dar; iki kat çıkıyoruz."],
    lose: ["Başka ilçeler de var; hepsinde arsa var.", "Pişman olacaksınız; projeyi saklıyorum.", "Seçimi kaybettim, iskeleyi bırakmam."],
  },
  kaan: {
    win: ["Karakavak 4.0 yüklendi; güncelleme sabah başlıyor.", "Bu bir win-win; en çok ben kazandım.",
      "Sinerjiyi sandığa taşıdık, sandık da bize.", "Yatırımcılar mutlu; seçmen de sonra mutlu olur."],
    lose: ["Pazar hazır değildi; 5.0'da görüşürüz.", "Yatırımcı küstü, ben de küstüm.", "Sunumu herkes beğendi, oyu kimse."],
  },
  bekir: {
    win: ["Kepenk değil bereket! Meclis yarın kıraathanede.", "Esnaf bunu unutmaz; veresiye defteri kapandı.",
      "Belediye çarşıya taşındı; çaylar benden.", "Bereketli olsun; meclis kararları okey masasında."],
    lose: ["Çarşıda konuşulur bu, hem de uzun uzun.", "Esnaf da bunu unutmaz.", "Kepenkleri açıyoruz; kırgınız ama açıyoruz."],
  },
  muhtar: {
    win: ["Önce Kavaklı, sonra gerisi; gerisini de sonra konuşuruz.", "Üç kez saydım, üçünde de kazandım.",
      "Kahvede anlatacağım; bu kez güzel şeyler.", "Mühür bende, tespih bende; hayırlı olsun."],
    lose: ["Kahvede anlatırım bunu.", "Parmak hesabıyla ben kazanmıştım.", "Kırk yıllık hatırı sandık da saymadı."],
  },
  tuncay: {
    win: ["Manşet hazır: Tuncay Başkan. İkinci baskı yolda.", "Kalemim sizden yanaydı; artık mühür de.",
      "Yarınki gazetede fotoğrafım büyük çıkacak.", "Bu haberi ben yazdım, siz de oyladınız."],
    lose: ["Bunu da yazarım, birinci sayfadan.", "Arşivime not düştüm.", "Manşet hazırdı; boşa gitmesin, başkasına basarım."],
  },
  burak: {
    win: ["Efsane! Hikâyeye atıyorum; belediyeyi de.", "Beğen, paylaş, oy ver; oy verdiniz, sağ olun!",
      "Canlı yayındayım, izleyici 14 oldu, rekor!", "Makam koltuğunda ilk içerik geliyor, takipte kalın."],
    lose: ["Takipten çıkıyorum; Karakavak'ı.", "Cringe bir sonuç ama yine de içerik.", "Algoritma bizi sevmedi."],
  },
  albay: {
    win: ["Saat tam dokuzda makamdayım; geç kalan olmasın.", "Nizam geldi, intizam yolda.",
      "Yaşa! Var ol Karakavak!", "İlk icraat: bütün saatler aynı saati gösterecek."],
    lose: ["Böyle sandık görmedim.", "Rahat! Beş yıl sonra yine hazır ol.", "Geri çekilmiyoruz, mevzi değiştiriyoruz."],
  },
  tekir: {
    win: ["Mırr.", "Mırrr. (Tercüme: Mama saatleri değişmeyecek.)",
      "Miyav. (Tercüme: Masadaki bardak yere düşecek.)", "Tısss. (Tercüme: Muhalefete selamlar.)"],
    lose: ["Tısss!", "Mırr. (Tercüme: Seneye yine buradayım.)", "(Sandığın üstünde uyumaya devam ediyor.)"],
  },
  _: { win: ["Hayırlı olsun Karakavak.", "Kazanan Karakavak oldu.", "Çaylar bizden."],
...
export const TV_KJ = {
  ...
  sayim: {
    title: [[v => v.mid, "Sandıkların {oranEk} açıldı"], [v => v.mid, "Sandıkların {oranEk} açıldı"],
      [v => v.mid, "Açılan sandık oranı {oran}"], [v => v.mid && v.leader, "{oran} açıldı, {lider} önde"],
      [v => v.mid, "Sayım sürüyor: {oran}"], [v => v.mid && v.early, "Erken seçimde {oran} açıldı"],
      [v => v.opened > 0 && v.opened < 0.15, "İlk sandıklar açıldı"],
      [v => v.opened > 0.8 && v.opened < 1, "Sayımda son viraj"],
      [v => v.youLead, "{you} önde"], [v => v.youBehind, "{lider} önde"],
      [v => v.close, "Başa baş yarış"], [v => v.gap >= 10, "Fark açılıyor"],
      [v => v.opened >= 1, "Sandıkların tamamı açıldı"], [v => v.opened === 0, "Sayım başladı"],
      "Karakavak'ta sayım sürüyor"],
    sub: ["Uzman stüdyoda: 'Kesin konuşmak için erken, yine de konuşacağım'",
  ...
  lider: {
    title: ["Liderlik el değiştirdi", "Liderlik el değiştirdi", [tvYes, "{lider} öne geçti"], "Zirvede değişiklik",
      [v => v.youLead, "{you} zirvede"], [v => v.prev === "you" && !v.youLead, "{you} liderliği kaptırdı"],
      [v => v.close, "Başa baş: sıra değişti"]],
    sub: ["{lider} öne geçti; stüdyodaki uzman gözlüğünü çıkarıp tekrar taktı",
      ...
      [v => v.prev && v.prev !== "you" && v.prev !== v.leader, "{prev} liderliği kaptırdı; ekibi pastayı buzdolabına geri koydu"],
      [v => v.close, "Fark yalnızca {fark} puan; stüdyoda nefesler tutuldu, çaylar soğudu"]],
  },
  mahalle: {
    title: ["{mahs} sandıkları açıldı", "{mahs} sandıkları açıldı", "Gözler {mahde}", "Sıra {mahde}",
    ...
    sub: [[tvYes, "{mahs} sandıkları geldi; tutanaklar masada, çaylar yolda", 0.4],
  ...
  son: {
    title: ["Son sandık yolda", "Gözler Yukarıkavak'ta", "Traktör bekleniyor", "Son sandık: Yukarıkavak",
      [v => v.opened >= 0.98 && v.opened < 1, "Sandıkların {oranEk} açıldı"],
      [v => v.close, "Her şey traktöre bağlı"]],
    sub: ["Yukarıkavak sandığı traktörle yolda; traktör çamura saplandı, muhtar sandığı omzunda getiriyor",
  ...
  sonuc: {
    title: [[v => !v.tekirWon, "Karakavak seçimini yaptı"], [v => !v.tekirWon, "Karakavak seçimini yaptı"],
      [v => !v.tekirWon, "Sandık konuştu"], [v => !v.tekirWon, "Karakavak kararını verdi"],
      [v => v.early, "Erken seçim sonuçlandı"], [v => v.early, "Erken seçimde sonuç belli"],
      [v => v.win, "{you} yeniden başkan"], [v => v.win, "Makam yerinde kaldı"], [v => v.win, "Çaylar yine belediyeden"],
      [v => v.win && v.margin < 1, "Kıl payı zafer"], [v => v.win && v.margin < 1, "Foto finiş"],
      [v => v.win && v.margin >= 25, "Sandıktan fark çıktı"], [v => v.win && v.res.you < 50, "Birinci birincidir"],
      [v => v.win && v.early, "Erken seçimde makam korundu"],
      [v => v.lost, "{kazanan} kazandı"], [v => v.lost, "Yeni başkan: {kazanan}"],
      [v => v.lost, "Makam el değiştirdi"], [v => v.lost, "Karakavak'ta değişim"],
      [v => v.lost && v.early && v.winner === "bekir", "Belediye çarşıya taşındı"],
      [v => v.tekirWon, "Tekir başkan!"], [v => v.tekirWon, "Karakavak'ın ilk tüylü başkanı"],
      [v => v.tekirWon, "Mırnav Partisi iktidarda"]],
    sub: [[v => v.win, "{you} {youf} ile yeniden seçildi; {nY} adaylı yarışta fark {fark} puan"],
...
export const TV_TICK = {
  secim: [
    ...
    [v => v.opened < 0.5 && v.a && v.b, "Sandıkların %3'ü açıkken {a} ve {b} ayrı ayrı zafer ilan etti; pastalar aynı pastaneden"],
    [v => v.has("tekir"), "Geçersiz oyların önemli kısmına kedi resmi çizilmiş; Mırnav Partisi 'niyet okunuyor' dedi"],
    [v => v.has("tekir"), "Tekir sandık kurulunun masasına yattı; tutanaklar bir saat gecikti, uyandırmaya kimse kıyamadı"],
    [v => v.has("tekir"), "Tekir'in afişi ilçede yırtılmayan tek afiş oldu; kimse elini kaldıramadı"],
    [v => v.has("burak"), "Burak, sandıkların %1'i açılmışken canlı yayında 'KAZANDIK' dedi; izleyici 12, biri kendisi"],
    ...
    [v => v.has("cengiz"), "Cengiz Bey sandık başına şantiye konteyneri kurdurdu: 'Kazanırsak ofis, kaybedersek çay ocağı'"],
    ...
    [v => v.has("tuncay"), "Karakavak Postası yarının manşetini iki ayrı baskıyla hazırladı; ikisinde de 'Tarihi Gece' yazıyor"],
    [v => v.has("albay"), "Nuri Bey sandık kuruluna yoklama aldı; 'burada' diyen müşahit 'mevcut' demediği için uyarıldı"],
    ...
    [v => v.opened >= 1 && v.leader, "Kesin olmayan sonuçlara göre {lider} kazandı; kahvede 'ben demiştim' diyenler seçmeni geçti"],
    ...
    [v => v.flags.itfaiye_muze, "Müzedeki 1974 model itfaiye aracı seçim gecesi için ışıklandırıldı; hâlâ yağ sızdırıyor"],
...
export function whyLines(res, playerName) {
  ...
  if (Number.isFinite(res.p0)) out.push(ek <= 0 ? (Math.abs(res.p0 - res.you) < 0.5
    ...
    : res.p0 >= 50 ? `Teke tek yarışsaydınız anket %${d(res.p0)} diyordu; ${TV_SAYI[ek] || ek} aday daha girince oylar bölündü.`
  ...
  else if (res.win && res.margin >= 25) out.push(`Fark ${d(res.margin)} puan; rakipler sandığı değil, takvimi suçluyor.`);
  ...
  if (res.fatigue) out.push(`${res.term}. dönem yorgunluğu: −${d(res.fatigue)} puan. Seçmen afişteki yüzünüzü ezberledi.`);
