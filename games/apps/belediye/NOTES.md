...
- 2. adım, parçalama: açılışta oyun kodu (gzip 27 KB), `kutuphane` (interact.js, 29 KB) ve `icerik` (kart metinleri, 35 KB) iniyor; üçü `modulepreload` ile paralel. Kod değişince metin, metin değişince kod önbellekte kalıyor. Seçim gecesi (`secim.js`: yayın metinleri ve spiker, 27 KB; stili 5,7 KB) seçim yaklaşınca arkadan iniyor, en geç yayın açılırken. Ad havuzu (7 KB) aday kaydı açılınca iniyor; havuz inmeden zara basılırsa atış havuz inince yapılıyor. Açılışta JS 123 KB'tan 91 KB'a, CSS 17 KB'tan 12,5 KB'a indi. Service worker bütün parçaları önbelleğe aldığı için çevrimdışı oyun da seçim gecesini açabiliyor.
- 3. adım, TypeScript: `src/` bütünüyle `.ts` (TypeScript 7, `strict`). Önce ortak tipler (`types.ts`) ve kart tabloları, sonra motor ve ad havuzu, seçim gecesi, en son `ui.ts`. Tipler yalnız silinecek sözdiziminden (`erasableSyntaxOnly`), bu yüzden Node araçları `.ts`'i doğrudan yüklüyor; sayfaya gömülen gösteriler `stripTypeScriptTypes` ile tipten arındırıyor. `pnpm typecheck` = `tsc --noEmit`.
- Tip denetimi üç şey buldu. Motorda davetin `switch` içinde varsayılana düşmesi açık bir yardımcıya çevrildi. Yayında fark bilinmiyorken `NaN` karşılaştırmaları sessizce yanlış çıkıyordu; şimdi açıkça sınanıyor. Seçim gecesinde lider yokken bir dizi `undefined` ile indeksleniyordu. Simülasyon sayıları değişmedi, derleme boyu da aynı kaldı.

## Lint ve biçim (2026-09-25)
Başkanın isteği: "bi tslintten bi prettierden geçir, özellikle sabitler korkunç bi dizilimde." TSLint 2019'da bırakıldı; yerine ESLint ve typescript-eslint.
- ESLint'in bulduğu gerçek artıklar az: araçlarda kullanılmayan `readFileSync` importları ve ölü bir `touch` yardımcısı. Geri kalanı kodun bilerek yaptığı iki alışkanlıktı: boş `catch {}` ve `a && b()` kısa yazımı. İkisine kuralda izin var.
- Prettier 120 sütunda. Kart tabloları, kişiler, SYN ve sonlar artık satır başına bir alan; kart başına tek uzun satır kalmadı. Ad havuzlarında Prettier her adı ayrı satıra koyuyordu (1500 satır), onlar `prettier-ignore` ile satır satır paketli.
...
- Ardından başkanın ikinci notu: "rastgele aday seçince isim aynı kalıyor." İki düğme ayrıldı. Rastgele aday başka bir hazır adayı adıyla birlikte getirir; kutu boşalır, silik yazı o adayın adıdır, çünkü broşürdeki lakap ve biyografi o ada ait. Ad zarı yalnız adı değiştirir. Vesikalığa elle tıklamak ada dokunmaz.

## Seçim beyannamesi ve açılış seçimi (2026-09-25)
Başkanın fikri: "Yeni oyuna başlarken bazı buff'lar olsun. Adayı seçtikten sonra rastgele bir seçim; kazanırsak buff'lı başlasın. Bu bir seçenek olacak: normalde doğrudan ama düşük başlar, risk alıp kazanırsa yüksek. Öncesinde vaatler olsun, oyunun mizahına uygun; oyun sürecinde de karşımıza çıksınlar." Kaybedince ne olacağını başkan seçti: oyun bitmesin, kıl payı kazanıp düşük başlansın.
- Akış: aday kaydı → seçim beyannamesi (altı söz, en çok üçü, canlı kampanya anketi) → sessiz kampanya ya da sandık. Sandık yolunda seçim gecesi yayını açılışta da oynar, oyuncu "Belediye başkan adayı" etiketiyle çıkar. Yayın ilk seçimi tanıyor ("yeni başkan", "seçildi"); "Neden?" satırı kampanya anketini ve sözleri anıyor. Sonra açılış evrakı gelir: sessiz zafer, ezici zafer ya da kıl payı; sözler tırnakla anılır.
- Sonuç baştan belli: şansa göre ya rahat zafer (fark 8-20) ya kıl payı (0,3-0,9). Kıl payında gece boyu geride kalıp sonunda öne geçmek, seçim gecesinin kendi sahnesi oldu.
- Motor ve test sırası: tipler, motor ve birim testleri (`test/kampanya.test.mjs`, zafer oranı 3000 atışta şansla ±%3); ardından ekran ve tarayıcı testi (`tools/kampanya.mjs`), sonra içerik ve denge. Kasa etkisi bir yazar ajanına yazdırıldı (18 söz), `ek-dene` vaat destekli hale geldi.
- Simülasyonun gösterdiği iki denge hatası:
  - Geçiştirilen söz iki kez ödeniyordu: defterde eksi yazılıyor, açık söz sayacı da sandıkta düşürüyordu. Artık hesap evrakı sözü her iki yolda kapatıyor.
  - Rahat zafer beş yılda sönüp sandıkta sessizden kötü çıkıyordu (%70'e %76). Zafer artık defterde "Sandıktan güçlü çıkış" +3 olarak da anılıyor.
- Son ölçüm ("insan" oyuncu, 1500 oyun, seçim kazanma): sessiz %76, rahat zafer %78, kıl payı %71, kumar ortalaması %75. Rastgele oynayan için risk aşağı yukarı nötr; sözünü tutan için kârlı. Kütük davası sonu %2'ye çıktı, çünkü geçiştirilen sözler skandal sayacını da artırıyor.

## Kampanya turu ve mühür (2026-09-26)
Başkanın şikâyeti: "Çok da bir fark göremedim. Sessiz gir, vaatsiz gir kazan, vaatle gir kazan; ne fark var, ne buff var?" Ardından: "Yaratıcı ol, araştır, mantıklı bir mekaniğe çevir. Riske değsin."
- **Teşhis (araştırma, `arastirma-kampanya-tasarim.md`):** Başlangıçtaki farklar göz önünde değildi. Göstergeler birkaç evrakta ortalamaya dönüyor, balayı çarpanı ise sayılarda saklı kalıyordu. Kumar da hiç kaybettirmiyordu, kötü sonucun kendine ait bir oyunu yoktu. Buff'ın hissedilmesi için oyuncunun eliyle kullandığı bir şey olmalı, kötü sonuç da bir görev getirmeli.
- **Mühür:** Kazananın elindeki jeton, düğmede sayısıyla durur. Basılan evrağın en ağır kaybını siler (en çok 10). Kayıp yoksa harcanmaz, 18 ayda söner. İlk sürümde yalnız halkı koruyordu. Simülasyon, ilk dönemde oyunu bitirenin çoğunlukla sandık (%45) ve kasa (%25) olduğunu, halkın dibe hiç vurmadığını gösterdi. Mühür bu yüzden en ağır kayba genelleştirildi.
- **Kampanya turu:** Sandığa gitmek artık bir karar dizisi. Dört evrakta kesin ile zarlı seçenek arasında seçilir, oran düğmede yazar. Evrakları bir yazar ajanı araştırmadaki 30 kampanya taktiğinden yazdı: seçim bürosu, şarkı, kapı kapı, çarşı turu, okey masası, köy, düğün, video, fenomen, açık oturum, anketçi, viral düşüş, müjde, seçim asfaltı, afiş savaşı, kapanış mitingi, kütük. Altı ilk dönem evrakı turun bıraktığı bayrakların hesabını soruyor.
- **Riskin ölçüsü:** Kazanma olasılığı anketle doğrusal olduğundan, beklenen değeri eşit iki seçenek arasında varyans bir şey değiştirmiyor. `kampanya=cesur` ile `temkinli` neredeyse aynı dağılımı verdi (kıl payı ikisinde de %38). Risk turda değil sandıkta: ezici eşiği, zarın anketin çok altına düşmesini ödüllendiriyor. Zarlı seçeneklerin beklenen değeri kesin seçenekten biraz yüksek (+0,4 ile +0,8), kötü uçları bayrak bırakıyor.
