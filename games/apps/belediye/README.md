...
| `src/cards.ts` | 32 karakter, 16 başkan adayı: ad, lakap, kısa biyografi (`BASKANLAR`), ~210 evrak (birbirine bağlı yaylar dahil), etkileşim tablosu (`SYN`), seçim adayları (`ADAYLAR`) ve seçmen grupları, kriz kartları, 11 son. Kart şeması dosyanın başında |
