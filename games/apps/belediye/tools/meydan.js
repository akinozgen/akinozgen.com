...
function meydanSVG() {
  const p = "md" + ++MD_N + "-",
    u = n => `url(#${p}${n})`; // gradyan kimlikleri her kopyada ayrı
  ...
  let sd = 20240917;
  const rnd = () => (sd = (sd * 16807) % 2147483647) / 2147483647; // her çağrıda aynı dizilim
  const SK = "#f7cba6",
    SKD = "#e8a883",
    INK = "#2b1a17",
    LIT = "#ffd772",
  ...
  const dots = n => {
    let d = "";
    for (let i = 0; i < n; i++) d += `M${Math.round(rnd() * 1980 - 30)} ${Math.round(24 + rnd() * rnd() * 320)}h0`;
    return d;
  };
  const spark = (x, y, r) =>
    `M${x} ${y - r}Q${x + r * 0.2} ${y - r * 0.2} ${x + r} ${y}Q${x + r * 0.2} ${y + r * 0.2} ${x} ${y + r}Q${x - r * 0.2} ${y + r * 0.2} ${x - r} ${y}Q${x - r * 0.2} ${y - r * 0.2} ${x} ${y - r}z`;
  const cloud =
    O(16, -16, 16) + O(42, -26, 24) + O(74, -20, 19) + O(98, -11, 12) + "M0 0h110a9 9 0 0 0 0 -18h-110a9 9 0 0 0 0 18z"; // defs'te bir kez
  const strip = cs =>
    rep(cs, ([x, y, s]) =>
      rep(
        [0, 2400],
        o =>
          `<use href="#${p}cld" class="md-fcl2" transform="translate(${x + o} ${y}) scale(${s})"/><use href="#${p}cld" class="md-fcl" transform="translate(${q(x + o + 5 * s)} ${q(y - 6 * s)}) scale(${Math.round(s * 93) / 100})"/>`,
      ),
    );

  // ── Tepeler ve çevre yolu (traktör bu doğru üzerinde gider)
  const rY = x => q(394 - ((x + 60) * 38) / 2050);
  const rows = rep([10, 21, 34, 50], d => `M-60 ${394 + d}L1990 ${356 + d}`),
    melons = rep([15, 27, 42], d => `M-60 ${394 + d}L1990 ${356 + d}`);
  const bushes = [
    ...[20, 90, 170, 250, 330, 420, 500, 590, 660],
    ...[1236, 1290, 1350, 1440, 1520, 1610, 1700, 1790, 1880, 1950],
  ]
    .map(x => `M${x} ${rY(x) - 6}h0`)
    .join("");
  ...
  [
    ...
    for (let r = 0; r < (h > 84 ? 2 : 1); r++)
      for (const fx of [0.28, 0.72]) {
        const d = R(Math.round(x + w * fx - 6), t + 16 + r * 30, 12, 16);
        wins += d;
        if (!((i + r + (fx > 0.5)) % 3)) lw1 += d;
      }
  });
  const poplar = (x, h, w, b = B1) =>
    `M${x} ${b - h}C${x + w} ${q(b - h * 0.72)} ${q(x + w * 0.9)} ${q(b - h * 0.2)} ${x + 2} ${b - 8}H${x - 2}C${q(x - w * 0.9)} ${q(b - h * 0.2)} ${x - w} ${q(b - h * 0.72)} ${x} ${b - h}z`;
  const plit = (x, h, w, b = B1) =>
    `M${x} ${b - h}C${x - w} ${q(b - h * 0.72)} ${q(x - w * 0.9)} ${q(b - h * 0.2)} ${x - 2} ${b - 8}H${x}z`;
  const POP = [
    [62, 150, 15],
    [118, 118, 13],
    [262, 168, 16],
  ...
  // ── Belediye binası
  const COLS = [862, 918, 1002, 1058],
    WU = [746, 798, 1092, 1144];
  ...
  const frames =
    ...
    R(1014, 383, 32, 56);
  const mull =
    rep(WU, x => `M${x + 15} 270V334M${x} 300H${x + 30}M${x + 15} 384V440M${x} 404H${x + 30}`) +
    "M890 276V334M877 302H903M1030 276V334M1017 302H1043M890 386V438M1030 386V438";
  const refl = rep(
    [...WU, 877, 1017],
    x => `M${x + 3} 330L${x + 13} 306V318L${x + 7} 330zM${x + 3} 436L${x + 13} 412V424L${x + 7} 436z`,
  );
  const n13 = [...Array(13).keys()],
    balc = rep(n13, i => R(911 + i * 8, 320, 3.6, 21)),
    roofB = rep(n13, i => R(732 + i * 8, 191, 3.5, 12) + R(1091 + i * 8, 191, 3.5, 12));
  ...
  const G = 36,
    FL = 54,
    fx = v => q(v * G);
  const flagD = `M0 0C${FL * 0.3} -3 ${FL * 0.62} 3 ${FL} 0V${G}C${FL * 0.62} ${G + 3} ${FL * 0.3} ${G - 3} 0 ${G}z`;
  const star =
    [180, 324, 108, 252, 36]
      .map(
        (a, i) =>
          `${i ? "L" : "M"}${q(0.82083 * G + 0.125 * G * Math.cos((a * Math.PI) / 180))} ${q(0.5 * G + 0.125 * G * Math.sin((a * Math.PI) / 180))}`,
      )
      .join("") + "z";
  const cres = `M${fx(0.71125)} ${fx(0.63369)}A${fx(0.25)} ${fx(0.25)} 0 1 1 ${fx(0.71125)} ${fx(0.36631)}A${fx(0.2)} ${fx(0.2)} 0 1 0 ${fx(0.71125)} ${fx(0.63369)}z`;
  ...
  const bunting = (x0, y0, x1, y1, sag, n, k) => {
    ...
    for (let i = 1; i < n; i++) {
      const t = i / n,
        x = q((1 - t) ** 2 * x0 + 2 * (1 - t) * t * cx + t * t * x1),
        y = q((1 - t) ** 2 * y0 + 2 * (1 - t) * t * cy + t * t * y1);
  ...
  const lamp = (x, b, t, s) =>
    P(`M${x - 12 * s} ${b}h${24 * s}l${-6 * s} ${-14 * s}h${-12 * s}z`, "#26302b") +
    `<path d="M${x} ${b - 12 * s}V${t + 10 * s}" stroke="#26302b" stroke-width="${5 * s}"/>` +
    ln(`M${x - 8 * s} ${t + 20 * s}Q${x} ${t + 12 * s} ${x + 8 * s} ${t + 20 * s}`, "#26302b", 2.2 * s) +
    P(`M${x - 6 * s} ${t - 9 * s}h${12 * s}l${-3 * s} ${-6 * s}h${-6 * s}z`, "#26302b") +
    `<ellipse class="md-glb" cx="${x}" cy="${t}" rx="${14 * s}" ry="${11 * s}"/>` +
    ln(
      `M${x - 13 * s} ${t}Q${x} ${t - 10 * s} ${x + 13 * s} ${t}M${x - 13 * s} ${t}Q${x} ${t + 10 * s} ${x + 13 * s} ${t}M${x} ${t - 10 * s}V${t + 10 * s}`,
      "#8fae45",
      2 * s,
    );
  const glow = (x, y, r) => `<circle cx="${x}" cy="${y}" r="${r}" fill="${u("gy")}"/>`;
  const tulip = (x, y, s = 1) =>
    `M${x - 3 * s} ${y}C${x - 3 * s} ${y + 3 * s} ${x - 1.5 * s} ${y + 4 * s} ${x - 1.5 * s} ${y + 5.5 * s}C${x - 1.5 * s} ${y + 7 * s} ${x - 3 * s} ${y + 8 * s} ${x - 3 * s} ${y + 10 * s}H${x + 3 * s}C${x + 3 * s} ${y + 8 * s} ${x + 1.5 * s} ${y + 7 * s} ${x + 1.5 * s} ${y + 5.5 * s}C${x + 1.5 * s} ${y + 4 * s} ${x + 3 * s} ${y + 3 * s} ${x + 3 * s} ${y}z`;
  const awning = (x, y, w, h, c) => {
    let a = R(x, y, w, h),
      b = "",
      sc = ["", ""];
    for (let i = 0; i * 8 < w; i++) {
      if (i % 2) b += R(x + i * 8, y, 8, h);
      sc[i % 2] += `M${x + i * 8} ${y + h}a4 4 0 0 0 8 0z`;
    }
    return P(a, "#f3f2ec") + P(b + sc[1], c) + P(sc[0], "#f3f2ec");
  };

  // lale tarhı: elips içinde sıra sıra lale (sap, kırmızı ve sarı taç)
  const tul = { st: "", r: "", y: "" };
  for (const cx of [1300, 622])
    for (let r = 0; r < 3; r++)
      for (let i = 0; i < 9 - r; i++) {
        const x = q(cx - 84 + r * 10 + (i * (168 - r * 20)) / (8 - r)),
          y = 894 + r * 7;
        tul.st += `M${x} ${y}v8`;
        tul[(i + r) % 3 ? "r" : "y"] += `M${x} ${y - 2}v-2`;
      }
  ...
  const pigeon = (
    ...
    flip,
    i,
  ) => `<g transform="translate(${x} ${y}) scale(${flip ? -s : s} ${s})"><g class="md-pg md-pg${(i % 3) + 1}"><g class="md-pgf">
...
${
  o.hat
    ? P(
        "M1477 655C1474 634 1492 625 1511 630C1520 633 1525 640 1527 647L1535 650C1530 655 1516 655 1507 653L1477 657z",
        "#57524b",
      ) + ln("M1479 650C1492 646 1510 646 1526 648", "#3f3b36", 2)
    : ln("M1481 648C1478 662 1482 672 1489 676", "#f1efe9", 5) +
      P("M1484 646C1488 640 1494 638 1500 639C1494 641 1489 645 1486 650z", "#f9dcc2")
}
...
<defs>
<linearGradient id="${p}sky" gradientUnits="userSpaceOnUse" x1="0" y1="-60" x2="0" y2="520">${V(0, "sk0")}${V(0.42, "sk1")}${V(0.78, "sk2")}${V(1, "sk3")}</linearGradient>
<linearGradient id="${p}hz" gradientUnits="userSpaceOnUse" x1="0" y1="250" x2="0" y2="500">${V(0, "sk3", 0)}${V(1, "sk3", 0.55)}</linearGradient>
<linearGradient id="${p}gd" gradientUnits="userSpaceOnUse" x1="0" y1="540" x2="0" y2="1080">${V(0, "sh", 0)}${V(1, "sh", 0.42)}</linearGradient>
<linearGradient id="${p}col">${V(0, "ws")}${V(0.3, "wt")}${V(0.65, "wc")}${V(1, "ws")}</linearGradient>
${RG("gy", S(0, "#fff3a0", 0.7), S(0.35, "#ffd98a", 0.28), S(1, "#ffd98a", 0))}${RG("sun", S(0, "#fffbe6", 0.9), S(0.3, "#fff2b8", 0.35), S(1, "#fff2b8", 0))}
${RG("dusk", S(0, "#ffe6a0", 0.95), S(0.18, "#ffb36b", 0.5), S(0.55, "#ff8a5c", 0.16), S(1, "#ff8a5c", 0))}${RG("moon", S(0, "#e8efff", 0.45), S(1, "#e8efff", 0))}
${RG("mkg", S(0, "#fff0bf", 1), S(0.3, "#ffd27a", 0.5), S(1, "#ffb04a", 0))}${RG("mkl", S(0, "#fffdf0", 0.95), S(0.45, "#fff3c8", 0.55), S(1, "#fff3c8", 0))}${RG("gold", S(0, "#fff3c0"), S(0.4, "#e8c664"), S(1, "#9a7428"))}${RG("fld", S(0, "#ffd98a", 0.34), S(1, "#ffd98a", 0))}
${L("mkw", S(0, "#ffd88c"), S(1, "#e0914a"))}${L("br", S(0, "#f3dc97"), S(0.5, "#c9a54c"), S(1, "#8a6a26"))}${L("kin", S(0, "#ffd58a"), S(1, "#c0703a"))}
${H("lsh", S(0, "#08111f", 0.55), S(1, "#08111f", 0))}${H("rip", S(0, "#fff", 0.14), S(0.25, "#000", 0.2), S(0.5, "#fff", 0.14), S(0.75, "#000", 0.2), S(1, "#fff", 0.14))}
<radialGradient id="${p}vig" cx=".56" cy=".44" r=".78">${S(0.55, "#060a18", 0)}${S(1, "#060a18", 0.5)}</radialGradient>
...
<g class="md-cl1 md-clo">${strip([
    [260, 150, 0.7],
    [980, 118, 0.55],
    [1620, 160, 0.8],
    [2130, 130, 0.6],
  ])}</g>
<g class="md-cl2 md-clo">${strip([
    [40, 262, 1.25],
    [700, 212, 0.95],
    [1330, 250, 1.4],
    [1960, 196, 1.05],
...
${lv(
  rep(
    ...
    ([a, b, c, d]) => rep([0, 7, 14, 21], k => `M${a + k} ${b + k}L${c + k * 2} ${d + k}`),
  ),
  "h2",
  2.6,
  " md-pch",
...
${f(
  "tr1",
  rep(POP, ([x, h, w]) => poplar(x, h, w)),
)}${f(
    "tr0",
    rep(POP, ([x, h, w]) => plit(x, h, w)),
  )}
...
${lv(
  rep([512, 534, 564, 602, 648, 702, 764, 834, 912, 998, 1092], y => `M-60 ${y}H1980`) +
    rep([...Array(38)], (_, i) => {
      const xb = -2000 + i * 160;
      return `M${q(960 + (xb - 960) * 0.2333)} 498L${xb} 1140`;
    }),
  "gr1",
  2,
  " md-pav",
...
<path d="${rep(COLS, x => R(x - 9, 262, 18, 190))}" fill="${u("col")}"/>${lv(
    rep(COLS, x => `M${x - 4} 267V448M${x + 4} 267V448`),
    "ws",
    1.4,
    " md-flu",
  )}
${f(
  ...
  rep(
    COLS,
    x => R(x - 14, 253, 28, 5) + R(x - 11, 258, 22, 4) + O(x - 11, 260, 3) + O(x + 11, 260, 3) + R(x - 12, 452, 24, 5),
  ),
)}
...
${f(
  "st",
  rep(STEPS, ([x, y, w]) => R(x, y, w, 3)),
)}${f(
    "wb",
    rep(STEPS, ([x, y, w, h]) => R(x, y + 3, w, h)),
  )}${f("sh", R(804, 506, 312, 4), " md-sho")}
...
<ellipse cx="0" cy="-31" rx="6.6" ry="4.6" fill="#fde7c8"/><g class="md-cey">${P(`M-7.3 -37a2.3 3 0 1 0 4.6 0a2.3 3 0 1 0 -4.6 0zM2.7 -37a2.3 3 0 1 0 4.6 0a2.3 3 0 1 0 -4.6 0z`, INK)}${P(O(-5.8, -38.2, 0.9) + O(4.2, -38.2, 0.9), "#fff")}</g>
${P("M-1.7 -33.6h3.4l-1.7 2.1z", "#e7888a")}${ln("M-2.6 -30.2q1.3 1.5 2.6 0q1.3 1.5 2.6 0", "#8a4a2a", 1)}${ln("M-7 -32l-9 -1.6M-7 -30.3l-9 1.6M7 -32l9 -1.6M7 -30.3l9 1.6", "#fff", 0.8)}</g>
...
${P(
  rep([1550, 1617, 1684], x => R(x - 9, 416, 8, 34) + R(x + 27, 416, 8, 34)),
  "#3d6b4a",
)}${P(
    rep([1550, 1617, 1684], x => R(x - 3, 450, 32, 5)),
    "#8a4a2a",
  )}${ln(
    rep([1550, 1617, 1684], x => `M${x + 2} 448h0M${x + 10} 447h0M${x + 18} 448h0M${x + 25} 447h0`),
    "#d8343a",
    4,
  )}
...
${rep(
  ...
  ([x, y]) =>
    `<ellipse cx="${x}" cy="${y + 4}" rx="118" ry="24" class="md-fwt"/><ellipse cx="${x}" cy="${y}" rx="108" ry="18" fill="#6b4a2e"/>`,
)}
...
<g class="md-pgs md-xde">${[
    ...
    .map(([x, y, s, fl], i) => pigeon(x, y, s, fl, i))
    .join("")}</g>
...
<g transform="translate(3160 0) scale(-1 1)">${man({
    ...
    cane: 1,
    arm: "md-armb",
    up: "M1504 694L1514 726",
    fore: "M1514 726L1530 722",
    hand: [1534, 720],
...
function meydanCSS() {
  const K =
    "sk0 sk1 sk2 sk3 h0 h1 h2 h3 rd cl cl2 hs0 hs1 hs2 rf gl tw tw2 tr0 tr1 wl ws wt wc wb st gr0 gr1 gr2 lg sh nt wa".split(
      " ",
    );
  const PAL = {
    ...
    aksam:
      "#2b3781 #7c5aa3 #ee8c63 #ffd08a #a2809a #86914c #6b7a3e #ecc868 #f0c690 #fbc79c #c07d8e #f0bb8f #e0a47f #f3cba0 #bc4a3b #7a7fae #efbd95 #c18876 #647f3e #465f31 #f6cc9d #d69a76 #ffe6bf #fbdcb0 #c19476 #e7b890 #d6a47d #bd8a66 #c99673 #fff1a2 #5a2f3a #3b3a2a #e9a48e",
  ...
  const t = (s, v) => `${s}{transform:${v}}`,
    kf = (n, b) => `@keyframes md-${n}{${b}}`,
    I = " infinite";
  ...
  let push = "";
  for (let i = 0; i <= 10; i++) {
    const e = (i / 10) ** 2 * (3 - (2 * i) / 10),
      s = SC ** e,
      x = WX + e * (960 - WX),
      y = WY + e * (540 - WY);
    push += t(
      i * 10 + "%",
      `translate(${(x - WX * s).toFixed(1)}px,${(y - WY * s).toFixed(1)}px) scale(${s.toFixed(3)})`,
    );
  }
  const fly = (n, a, b, c) =>
    kf(
      "fly" + n,
      `0%,100%{transform:none}15%{transform:translate(${a})}50%{transform:translate(${b})}80%{transform:translate(${c})}`,
    );
  return (
    Object.entries(PAL)
      .map(
      ...
      .join("") +
    K.map(k => `.md-f${k}{fill:var(--md-${k})}`).join("") +
    "h2 h3 rd tr1 gr1 ws wt nt"
      .split(" ")
      .map(k => `.md-k${k}{stroke:var(--md-${k})}`)
      .join("") +
    "sk0 sk1 sk2 sk3 ws wt wc sh"
      .split(" ")
      .map(k => `.md-s${k}{stop-color:var(--md-${k})}`)
      .join("") +
    `
...
function meydan(rootEl, opts = {}) {
  ...
  const restart = (c, el = svg) => {
    on(c, false, el);
    void svg.getBoundingClientRect();
    on(c, true, el);
  };
  const $ = s => svg.querySelector(s),
    pigeons = [...svg.querySelectorAll(".md-pg")],
    flock = $(".md-pgs");
  const layers = [0, 1, 2, 3].map(i => $(".md-l" + i)),
    DEPTH = [0.18, 0.36, 0.62, 1]; // en yakın katman 18 px
  ...
  const loops = () => {
    const blinkL = () =>
      later(
        () => {
          blink();
          if (Math.random() < 0.2) later(blink, 300);
          blinkL();
        },
        rnd(2500, 6500),
      );
    ...
    const tavlaL = () =>
      later(
        () => {
          const c = turn++ % 2 ? "md-tb" : "md-ta";
          restart(c);
          later(() => on(c, false), 1400);
          tavlaL();
        },
        rnd(3200, 6000),
      );
    const peckL = () =>
      later(
        () => {
          if (pal !== "gece" && pigeons.length) {
            const g = pigeons[(Math.random() * pigeons.length) | 0];
            on("md-pk", true, g);
            later(() => on("md-pk", false, g), 220);
          }
          peckL();
        },
        rnd(500, 1800),
    ...
    const flyL = () =>
      later(
        () => {
          if (pal !== "gece") {
            restart("md-fly", flock);
            later(() => on("md-fly", false, flock), 2800);
          }
          flyL();
        },
        rnd(14000, 26000),
    ...
    const trkL = first =>
      later(
        ...
        first ? rnd(5000, 9000) : rnd(40000, 60000),
      );
    blinkL();
    earL();
    tavlaL();
  ...
  const paint = () =>
    layers.forEach((g, i) =>
      g?.setAttribute(
        "transform",
        `translate(${(-cx * 18 * DEPTH[i]).toFixed(2)} ${(-cy * 10 * DEPTH[i]).toFixed(2)})`,
      ),
    );
  const step = () => {
    raf = 0;
    cx += (tx - cx) * 0.08;
    cy += (ty - cy) * 0.08;
    if (Math.abs(tx - cx) + Math.abs(ty - cy) < 0.003) {
      cx = tx;
      cy = ty;
  ...
  const kick = () => {
    if (!raf && typeof requestAnimationFrame === "function") raf = requestAnimationFrame(step);
  };
  const move = e => {
    if (!live || entered || enterDone || rm() || !fine?.matches) return;
  ...
  const enter = () =>
    new Promise(res => {
      ...
      const quick = rm();
      on("md-pz", false);
      restart(quick ? "md-inr" : "md-in"); // duraklatılmış sahnede de oynasın
      enterDone = res;
      setTimeout(
        () => {
          if (enterDone !== res) return;
          enterDone = null;
          entered = true;
          apply();
          on("md-pz");
