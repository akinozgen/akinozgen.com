...
try {
  ...
  const tr = () =>
    ev(
      `(() => { const t = document.querySelector(".ongo-track"); return t.className + " " + getComputedStyle(t).transform; })()`,
    );
  console.log("şerit:", await tr());
  await sleep(1000);
  console.log("şerit:", await tr());
  await ev(`document.querySelector("#btn-danis").click()`);
  await sleep(400);
  await shot("s0-fikret");
  console.log("Fikret:", await ev(`document.querySelector("#note span")?.textContent`));
  await ev(`document.querySelector("#note")?.click()`);
  await sleep(200);
  const b = await ev(
    `(() => { const r = document.querySelector("#ch-R").getBoundingClientRect(); return [r.x + r.width / 2, r.y + r.height / 2]; })()`,
  );
  await send("Input.dispatchMouseEvent", { type: "mouseMoved", x: b[0], y: b[1] });
  await sleep(400);
