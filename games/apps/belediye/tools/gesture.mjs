...
async function touchPath(path) {
  ...
  for (const [x, y, w] of path.slice(1)) {
    await sleep(w);
...
const line = (x0, y0, dx, dy, steps, ms) => [
  [x0, y0, 0],
  ...Array.from({ length: steps }, (_, i) => [x0 + (dx * (i + 1)) / steps, y0 + (dy * (i + 1)) / steps, ms]),
];
...
try {
  ...
  for (let t = 0; t < 50 && !url; t++) {
    try {
      url = (await (await fetch("http://127.0.0.1:9340/json/list")).json()).find(
        x => x.type === "page",
  ...
  await trial(
    "kısa yavaş sürükleme (60px, 600ms, bekle)",
    ([x, y]) => touchPath([...line(x, y, -60, 3, 10, 60), [x - 60, y + 3, 250]]),
    false,
  );
  ...
  await trial(
    "uzun yavaş sürükleme (150px)",
    ([x, y]) => touchPath([...line(x, y, 150, 5, 15, 40), [x + 150, y + 5, 200]]),
    true,
  );
