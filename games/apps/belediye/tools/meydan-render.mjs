...
export const W = 1344,
  H = 752; // SDXL'in 16:9'a en yakın kovası (8'in katı); çıktı bunun iki katı
...
const ev = async e => {
  ...
  if (r.exceptionDetails) errors.push(e + " → " + r.exceptionDetails.text);
...
try {
  ...
  await send("Page.navigate", { url: new URL("file:///" + OUT.replace(/\\/g, "/") + "/render.html").href });
  await sleep(1200);
  await ev(`document.fonts.ready.then(() => 1)`);
  for (const pal of ["aksam", "gece", "gun"]) {
    await ev(`kur(${JSON.stringify(pal)})`);
    await sleep(300);
    await send("Emulation.setDeviceMetricsOverride", { width: W, height: H, deviceScaleFactor: 1, mobile: false });
    await send("Emulation.setDefaultBackgroundColorOverride", {});
    await ev(`document.documentElement.className = "base"`);
    await sleep(250);
    await shot(`meydan-${pal}-base`);
    await send("Emulation.setDeviceMetricsOverride", { width: W, height: H, deviceScaleFactor: 2, mobile: false });
    await ev(`document.documentElement.className = ""`);
    await sleep(250);
    await shot(`meydan-${pal}-full@2x`);
    await send("Emulation.setDefaultBackgroundColorOverride", { color: { r: 0, g: 0, b: 0, a: 0 } });
    await ev(`document.documentElement.className = "over"`);
    await sleep(250);
    await shot(`meydan-${pal}-over@2x`);
  }
} catch (e) {
