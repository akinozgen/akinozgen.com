...
const key = k =>
  ev(
    `document.dispatchEvent(new KeyboardEvent("keydown",{key:${JSON.stringify(k)},bubbles:true})) || window.dispatchEvent(new KeyboardEvent("keydown",{key:${JSON.stringify(k)}}))`,
  );
...
try {
  ...
  await ev(`document.querySelectorAll("#picks .pick")[2].click()`);
  await sleep(400);
  const av = JSON.parse(
    await ev(
      `JSON.stringify({ ls: localStorage.getItem("cb.avatar"), ph: document.querySelector("#in-name").placeholder, pick: document.querySelectorAll("#picks .pick")[2].getAttribute("aria-label"), same: document.querySelector("#avatar-img").src === document.querySelectorAll("#picks .pick img")[2].src, on: document.querySelectorAll("#picks .pick")[2].getAttribute("aria-checked") === "true", scr: !document.querySelector("#scr-pick").hidden })`,
    ),
  );
  if (av.ls !== '"baskan-03"' || av.ph !== av.pick || !av.same || !av.on || !av.scr)
  ...
  const r = await ev(
    `(() => { const b = document.querySelector("#card").getBoundingClientRect(); return [b.x + b.width/2, b.y + b.height/2]; })()`,
  );
  await mouse("mousePressed", r[0], r[1]);
  await mouse("mouseMoved", r[0] - 40, r[1]);
  await mouse("mouseMoved", r[0] - 95, r[1] + 4);
  await sleep(250);
  await shot("3-surukle-telefon");
  await mouse("mouseMoved", r[0] - 180, r[1] + 6);
  await mouse("mouseReleased", r[0] - 180, r[1] + 6);
  await sleep(620);
  ...
  while (n++ < 320) {
    ...
    if (scr === "secim") {
      ...
      await key("Enter");
      await sleep(900);
      console.log("seçim gecesi geçildi");
      continue;
    }
    if (scr !== "game") break;
    if (n === 14) {
      ...
      await ev(`document.querySelector("#btn-log").click()`);
      await sleep(450);
      await shot("4b-gunluk-telefon");
      const lg = JSON.parse(
        await ev(
          `JSON.stringify({ open: document.querySelector("#log").classList.contains("open"), n: document.querySelectorAll("#log-list .lg-e").length })`,
        ),
      );
  ...
  if (!hall.some(r => r.avatar === "baskan-03"))
    errors.push("TEST: duvara seçilen vesikalık yazılmadı " + JSON.stringify(hall.map(r => r.avatar)));

  await viewport(1280, 820, false);
  await ev(`document.querySelector("#btn-back").click()`);
  await sleep(400);
  await ev(`document.querySelector("#btn-again").click()`);
  await sleep(1200);
  ...
  const sw = await ev(
    `(async () => { const r = await navigator.serviceWorker?.getRegistration(); const k = self.caches ? await caches.keys() : []; const fonts = [...document.fonts].filter(f => f.status === "loaded").map(f => f.family.replace(/"/g, "") + " " + f.weight + (f.style === "italic" ? "i" : "")); return JSON.stringify({ sw: !!r, active: !!r?.active, caches: k, fonts: [...new Set(fonts)] }); })()`,
  );
