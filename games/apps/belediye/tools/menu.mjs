...
const shot = async name => { const { data } = await send("Page.captureScreenshot", { format: "png" }); writeFileSync(`${OUT}/${name}.png`, Buffer.from(data, "base64")); };
const key = k => ev(`document.activeElement.dispatchEvent(new KeyboardEvent("keydown",{key:${JSON.stringify(k)},bubbles:true}))`);
const check = (ok, msg) => { if (!ok) errors.push("TEST: " + msg); };
const screenNow = () => ev(`["title","game","over","wall","help","pick","secim"].find(x => !document.querySelector("#scr-" + x).hidden)`);
...
const layout = () => ev(`JSON.stringify((() => {
...
const save = () => { const s = E.newGame(); s.month = 26; s.m = { h: 62, k: 12, e: 50, a: 71 }; E.draw(s, Math.random); return s; };
const fresh = async (withSave, w, h, mobile) => {
  ...
  await ev(`localStorage.clear(); localStorage.setItem("cb.introSeen", "true"); localStorage.setItem("cb.avatar", '"baskan-05"');
    ...
    location.reload()`); await sleep(1600);
};
try {
  ...
  for (const [w, h, mob] of [[390, 844, true], [360, 640, true], [1280, 800, false], [1920, 1080, false], [2560, 1080, false]]) {
  ...
  await key("ArrowDown");
  check(await onItem() === "btn-start" && /Yeni dönem/.test(await ev(`document.querySelector("#mn-info h3")?.textContent`)), "↓ ile Yeni dönem seçilmedi");
  ...
  const saveCard = await ev(`(document.querySelector("#btn-resume").focus(), document.querySelector("#mn-info .mi-meters")?.children.length || 0)`);
  ...
  check(await ev(`document.querySelectorAll("#mn-info .mi-wall li").length`) === 2, "bilgi kartında eski başkanlar listesi yok");
  // ayarlar: ses düzeyi ve titreşim kalıcı, Escape paneli kapatır ve odağı geri verir
  await ev(`(b => { b.focus(); b.click(); })(document.querySelector("#btn-ayar"))`); await sleep(400);
  check(await ev(`!document.querySelector("#mn-ayar").hidden && document.querySelector("#mn-list").inert`), "ayarlar paneli açılmadı");
  await shot("menu-ayarlar");
  await ev(`(() => { const r = document.querySelector("#ay-vol"); r.value = 30; r.dispatchEvent(new Event("input", { bubbles: true })); const t = document.querySelector("#ay-tit"); t.click(); })()`);
  check(await ev(`localStorage.getItem("cb.volume") === "30" && localStorage.getItem("cb.vibrate") === "false"`), "ses düzeyi ya da titreşim kaydedilmedi");
  // kayıtları sil: ilk basış yalnız uyarır
  await ev(`document.querySelector("#ay-sil").click()`);
  check(await ev(`!!localStorage.getItem("cb.save") && document.querySelector("#ay-sil").classList.contains("armed")`), "ilk basışta silindi ya da uyarmadı");
  await key("Escape"); await sleep(300);
  check(await ev(`document.querySelector("#mn-ayar").hidden && document.activeElement.id === "btn-ayar"`), "Escape paneli kapatmadı ya da odak geri gelmedi");
  await ev(`(b => { b.focus(); b.click(); })(document.querySelector("#btn-kunye"))`); await sleep(400);
  ...
  const ak = JSON.parse(await ev(`JSON.stringify({ ls: JSON.parse(localStorage.getItem("cb.avatar")), on: document.querySelector('#picks [aria-checked="true"]')?.dataset.id, img: document.querySelector("#avatar-img").src.includes("baskan-06") || document.querySelector("#avatar-img").src.startsWith("data:"), bio: document.querySelector("#ak-bio").textContent.length })`));
  check(ak.ls === "baskan-06" && ak.on === "baskan-06" && ak.img && ak.bio > 20, `→ vesikalığı değiştirmedi ${JSON.stringify(ak)}`);
  await shot("aday-1280");
  await ev(`(() => { const i = document.querySelector("#in-name"); i.value = "Test Başkan"; i.dispatchEvent(new Event("input", { bubbles: true })); })()`);
  await ev(`document.querySelector("#btn-go").click()`); await sleep(900);
  check(await screenNow() === "game" && await ev(`document.querySelector("#leader-name").textContent`) === "Test Başkan", "mazbata oyunu başlatmadı ya da ad yanlış");
  ...
  await ev(`document.querySelector("#btn-resume").click()`); await sleep(1600);
