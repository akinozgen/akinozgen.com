...
const setup = () => {
  const s = E.newGame();
  Object.assign(s.m, { h: 52, k: 44, e: 33, a: 47 });
  s.month = 59;
  s.fieldTerm = 1;
  s.cnt.tekir = 3;
  s.rel = { bekir: -2 };
...
const load = async s => {
  ...
  await sleep(1200);
  await ev(
    `localStorage.clear(); localStorage.setItem("cb.save", ${JSON.stringify(JSON.stringify(s))}); localStorage.setItem("cb.introSeen", "true"); location.reload()`,
  );
  await sleep(1400);
  await ev(
...
try {
  ...
  for (const [w, h, mob] of [
    ...
    const live = JSON.parse(
      await ev(`JSON.stringify({ open: parseFloat(document.querySelector("#tv-open").textContent.slice(1).replace(",", ".")),
    ...
    check(
      live.order.every((x, i) => !i || live.order[i - 1].v >= x.v),
      `${w}: adaylar oya göre dizilmemiş ${JSON.stringify(live.order)}`,
    );
    ...
    for (const c of fin.res.cands) {
      ...
      check(
        row && row.pct === "%" + c.pct.toFixed(2).replace(".", ","),
        `${w}: ${c.id} ekranda ${row?.pct}, motorda ${c.pct}`,
      );
    }
    ...
    check(
      fin.rows
        .filter(r => r.won)
        .map(r => r.id)
        .join() === fin.res.winner && fin.rows[0].lead,
      `${w}: mühür yanlış satırda`,
    );
    ...
    check(
      (await screenNow()) === "game" && /Seçim sonucu/.test(after),
      `${w}: Devam'dan sonra seçim sonucu kartı gelmedi (${after})`,
    );
  ...
  const rm = JSON.parse(
    await ev(
      `JSON.stringify({ go: !document.querySelector("#btn-ec-go").hidden, open: document.querySelector("#tv-open").textContent, won: document.querySelector(".tv-cand.won")?.dataset.id, res: JSON.parse(localStorage.getItem("cb.save")).pending.res.winner })`,
    ),
  );
  ...
  await load(mid);
  await ev(`document.dispatchEvent(new KeyboardEvent("keydown",{key:"a",bubbles:true}))`);
  await sleep(1400);
