...
const ev = async e => {
  ...
  if (r.exceptionDetails)
    errors.push("İFADE: " + e + " → " + (r.exceptionDetails.exception?.description || r.exceptionDetails.text));
...
const shot = async (name, clip) => {
  ...
  console.log("görüntü:", name);
};
// 1920×1080'de CSS pikseli = viewBox birimi: yakın plan kutuları doğrudan sahne koordinatı
const CLIPS = {
  bina: { x: 700, y: 120, width: 530, height: 400, scale: 2 },
  makam: { x: 900, y: 240, width: 120, height: 130, scale: 5 },
  tekir: { x: 920, y: 400, width: 100, height: 80, scale: 5 },
  amcalar: { x: 1420, y: 600, width: 320, height: 220, scale: 3 },
  ocak: { x: 1200, y: 350, width: 230, height: 190, scale: 3 },
  anit: { x: 740, y: 660, width: 440, height: 300, scale: 2 },
  sag: { x: 1400, y: 160, width: 520, height: 400, scale: 2 },
};
const view = (width, height, dpr = 1, mobile = false) =>
...
try {
  ...
  ws.addEventListener("message", m => {
    ...
    if (d.method === "Runtime.consoleAPICalled" && d.params.type === "error")
      errors.push("KONSOL: " + d.params.args.map(a => a.value ?? a.description).join(" "));
  });
  await send("Runtime.enable");
  await send("Page.enable");
  await view(1920, 1080);
  await send("Page.navigate", { url: pathToFileURL(OUT + "/demo.html").href });
  await sleep(900);
  ...
  console.log(
    ...
    info.sign,
    "birim (bant 464) · SVG",
    (info.bytes / 1024).toFixed(1),
    "KB · meydan.js",
    (Buffer.byteLength(SRC) / 1024).toFixed(1),
    "KB",
  );
  ...
  for (const [w, h, dpr, mob] of SIZES) {
    await view(w, h, dpr, !!mob);
    await sleep(250);
    for (const [pal, hr] of Object.entries(HOURS)) {
      await ev(`M.setHour(${hr})`);
      await sleep(350);
  ...
  await view(1920, 1080);
  await ev(`document.getElementById("menu").hidden = true; M.setHour(18)`);
  await sleep(300);
  await shot("bos-1920x1080-aksam");
  await ev(`M.setHour(23)`);
  await sleep(300);
  await shot("bos-1920x1080-gece");
  await ev(`M.setHour(11)`);
  await sleep(300);
  await shot("bos-1920x1080-gun");
  // Yakın planlar (boşta, menüsüz)
  for (const pal of ["aksam", "gece", "gun"]) {
    await ev(`M.setHour(${HOURS[pal]})`);
    await sleep(150);
  ...
  await ev(
    `(() => { const s = document.querySelector("svg.md-root"); s.querySelector(".md-pgs").classList.add("md-fly"); s.classList.add("md-ta", "md-trkgo", "md-blink"); })()`,
  );
  await sleep(60);
  await ev(
    `document.getAnimations().forEach(a => { const n = a.animationName; a.pause(); a.currentTime = n === "md-trk" ? 7000 : n === "md-fly1" || n === "md-fly2" || n === "md-fly3" ? 1100 : n === "md-flap" ? 60 : n === "md-shake" || n === "md-dice" ? 820 : a.currentTime; }), 1`,
  );
  await shot("canli-1920x1080");
  await shot("canli-amcalar", CLIPS.amcalar);
  ...
  await send("Input.dispatchMouseEvent", { type: "mouseMoved", x: 1900, y: 1060 });
  await sleep(1400);
  const tr = await ev(`[0,1,2,3].map(i => document.querySelector(".md-l" + i).getAttribute("transform"))`);
  console.log("parallaks:", tr.join(" | "));
  if (!tr[3] || !/translate\(-1[5-8]/.test(tr[3]))
    errors.push("TEST: parallaks en yakın katmanda ~18 px değil: " + tr[3]);
  await send("Input.dispatchMouseEvent", { type: "mouseMoved", x: 960, y: 540 });
  await sleep(1400);
  ...
  const cpu = async () => {
    ...
    return Math.round(((b.TaskDuration - a.TaskDuration) / (b.Timestamp - a.Timestamp)) * 1000) / 10;
  };
  const busy = await cpu();
  await ev(`M.pause()`);
  const idle = await cpu();
  await ev(`M.resume()`);
  ...
  await ev(`document.getElementById("menu").hidden = true; window.P = M.enter(), 1`);
  await sleep(30);
  for (const [i, t] of [
    [1, 200],
    [2, 450],
    ...
    await ev(
      `document.getAnimations().forEach(a => { if (/^md-(push|fl|mkg|mkb)$/.test(a.animationName)) { a.pause(); a.currentTime = ${t}; } }), 1`,
    );
    await sleep(80);
    await shot(`enter-${i}`);
  }
  await ev(
    `document.getAnimations().forEach(a => { if (/^md-(push|fl|mkg|mkb)$/.test(a.animationName)) a.finish(); }), 1`,
  );
  await ev(`document.getElementById("menu").hidden = false`);
  const t0 = Date.now();
  await ev(`P`);
  console.log("enter() çözüldü:", Date.now() - t0, "ms sonra (kare çekiminden sonra)");
  ...
  const dt = await ev(
    `(() => { const t = performance.now(); return M.enter().then(() => Math.round(performance.now() - t)); })()`,
  );
