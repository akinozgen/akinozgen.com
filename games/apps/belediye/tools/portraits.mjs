...
const CHROME = process.env.CHROME || "C:/Program Files/Google/Chrome/Application/chrome.exe", PORT = 9339;
const OUT = fileURLToPath(new URL("../.cache/portraits/", import.meta.url));
const chrome = spawn(CHROME, ["--headless=new", "--mute-audio", "--disable-gpu", `--remote-debugging-port=${PORT}`, `--user-data-dir=${OUT}profile`, "--no-first-run", "about:blank"], { stdio: "ignore" });
const sleep = ms => new Promise(r => setTimeout(r, ms));
let ws, id = 0; const pending = new Map();
const send = (method, params = {}) => new Promise((res, rej) => { const i = ++id; pending.set(i, { res, rej }); ws.send(JSON.stringify({ id: i, method, params })); });
// SVG'yi tuvale çizip WebP'ye çevirir
const render = async svg => {
  const r = await send("Runtime.evaluate", { awaitPromise: true, returnByValue: true, expression: `new Promise((ok, no) => {
  ...
  if (!url?.startsWith("data:image/webp;base64,")) throw new Error(r.exceptionDetails?.exception?.description || "webp üretilemedi");
