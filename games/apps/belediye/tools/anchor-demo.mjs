...
try {
  ...
  const info = await ev(
    `(() => { const s = document.querySelector("#big svg"); return { nodes: s.querySelectorAll("*").length, tick: Math.round(s.querySelector(".st-tick").getComputedTextLength()) }; })()`,
  );
  console.log(
    "SVG düğüm:",
    info.nodes,
  ...
  await sleep(300);
  await shot("00-bos", FULL);
  await shot("00-bos-yuz", FACE);
  // boşta davranışların kareleri: göz kırpma ve kartlara bakış (sınıf elle verilir)
  await ev(`document.querySelector("#big svg").classList.add("st-blink")`);
  await sleep(120);
  await shot("00-kirpma-yuz", FACE);
  await ev(`document.querySelector("#big svg").classList.replace("st-blink", "st-glance")`);
  await sleep(450);
  await shot("00-kartlara-bakis-yuz", FACE);
  await ev(`document.querySelector("#big svg").classList.remove("st-glance")`);
  await sleep(400);

  // Konuşma: farklı anlarda farklı ağız şekilleri
  await ev(`both("talk", true)`);
  for (let i = 1; i <= 4; i++) {
    await sleep(130 + i * 37);
    await shot(`01-konusma-${i}`, FACE);
  }
  await shot("01-konusma-tam", FULL);
  await ev(`both("talk", false)`);
  await sleep(300);

  for (const m of ["neutral", "excited", "shocked", "happy", "sad", "smug"]) {
    await ev(`both("mood", "${m}")`);
    await sleep(750);
    await shot(`02-mod-${m}`, FULL);
    await shot(`02-mod-${m}-yuz`, FACE);
  }
  await ev(`both("mood", "neutral")`);
  await sleep(500);

  await ev(`both("react", "point")`);
  await sleep(750);
  await shot("03-point", FULL);
  await sleep(1400);
  await ev(`both("react", "papers")`);
  await sleep(190);
  await shot("03-papers", FULL);
  await sleep(1100);
  await ev(`both("react", "lean")`);
  await sleep(700);
  await shot("03-lean", FULL);
  await sleep(1800);
  await ev(`both("react", "cat")`);
  await sleep(1300);
  await shot("03-cat-1-yuruyor", FULL);
  await sleep(1850);
  await shot("03-cat-2-kalem", FULL);
  await sleep(1500);
  await shot("03-cat-3-donus", FULL);
  await sleep(3000);
  await ev(`both("react", "lights")`);
  await sleep(125);
  await shot("03-lights-1", FULL);
  await sleep(160);
  await shot("03-lights-2", FULL);
  await sleep(1500);

  await ev(`both("setWall", "Sandıkların %37'si açıldı")`);
  await sleep(650);
  await shot("04-duvar", FULL);
  await ev(`both("setWall", "Liderlik el değiştirdi: Hacı Bekir öne geçti, çarşıda kepenkler açıldı")`);
  await ev(`both("year", 2039)`);
  await sleep(650);
  await shot("04-duvar-uzun", FULL);
  await ev(`both("mood", "excited"); both("say", 1500)`);
  await sleep(420);
  await shot("05-heyecanli-konusma", FULL);
  await sleep(1400);
  ...
  await ev(`both("talk", true)`);
  await sleep(400);
  const a = await ev(`document.querySelector("#big svg").getAttribute("class")`);
  await sleep(500);
  const b = await ev(`document.querySelector("#big svg").getAttribute("class")`);
  if (a !== b || !/st-v-e/.test(a) || !/st-rm/.test(a))
    errors.push(`TEST: hareket azaltmada ağız sabit değil (${a} → ${b})`);
  await shot("06-azaltma-konusma", FULL);
  await ev(`both("react", "cat")`);
  await sleep(300);
  await shot("06-azaltma-kedi", FULL);
