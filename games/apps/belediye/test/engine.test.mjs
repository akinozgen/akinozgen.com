...
test("zincirler, işler ve ilişkiler var olan kartlara ve kişilere bağlı", () => {
  ...
  for (const [k, a] of Object.entries(E.ACILIS)) if (a.next) link({ id: "ACILIS " + k }, a.next.id); // göreve başlarken gelir
...
test("her kişinin ve başkanlık vesikalığının resmi var, başıboş resim yok", () => {
  ...
  for (const f of readdirSync(dir))
    assert.ok(ids.includes(f.replace(/\.webp$/, "")), `${f} hiçbir kişiye ya da BASKANLAR'a bağlı değil`);
});
