...
const ySoy = new Set(A.YASAK_SOYAD.map(lc)), yTam = new Set(A.YASAK_TAM.map(lc)), yAd = new Set(A.YASAK_AD.map(lc));
...
test("havuzlar geniş, tekrarsız", () => {
  ...
  assert.deepEqual(dups(A.ADLAR.k), []); assert.deepEqual(dups(A.ADLAR.e), []); assert.deepEqual(dups(A.SOYADLAR), []);
...
test("yazım: her ad ve soyad Türkçe harflerle, baş harfi büyük (İ/ı doğru)", () => {
  ...
  for (const w of ["Işık", "Irmak", "Ilgın", "Ilgaz", "Işıl"]) assert.ok([...A.ADLAR.k, ...A.ADLAR.e, ...A.SOYADLAR].includes(w), w);
...
test("gerçek kişi yok: yasak soyad, yasak ad ve rakip adayların adları havuzda değil", () => {
  assert.deepEqual(A.SOYADLAR.filter(s => ySoy.has(lc(s))), []);
  assert.deepEqual([...A.ADLAR.k, ...A.ADLAR.e].filter(a => yAd.has(lc(a))), []);
  for (const s of ["Erdoğan", "Kılıçdaroğlu", "İmamoğlu", "Yavaş", "Bahçeli", "Akşener", "Babacan", "Davutoğlu", "Demirtaş", "İnce", "Soylu",
    "Özel", "Gül", "Sezer", "Bayar", "İnönü", "Menderes", "Ecevit", "Demirel", "Özal", "Çiller", "Erbakan", "Türkeş", "Baykal", "Atatürk"])
  ...
  const rakip = [...Object.keys(A.ADAYLAR).flatMap(k => A.PEOPLE[k].ad.split(" ")).filter(w => !unvan.has(lc(w))), "Fikret"];
  ...
  for (const b of Object.values(A.BASKANLAR)) assert.ok(!A.SOYADLAR.includes(b.ad.split(" ").at(-1)), `BASKANLAR soyadı havuzda: ${b.ad}`);
...
test("20 bin ad: yasak birleşim yok, cins havuzu tutuyor, kutuya sığıyor", () => {
  ...
  for (const cins of ["k", "e"]) {
    ...
    for (let i = 0; i < 10000; i++) {
      const ad = A.rastgeleAd(cins, r, son), [a, s, ...rest] = ad.split(" ");
...
test("ünlü tam adlar imkânsız: zar tam o ada düşse bile başka ad gelir", () => {
  ...
  for (const t of A.YASAK_TAM) {
    const [a, s] = t.split(" ");
    for (const cins of ["k", "e"]) {
      const ia = A.ADLAR[cins].indexOf(a), is = A.SOYADLAR.indexOf(s);
      if (ia < 0 || is < 0) continue;
      n++;
      // ilk atış tam yasaklı ada düşsün; sonrası sabit tohumla
      const seq = [(ia + .5) / A.ADLAR[cins].length, (is + .5) / A.SOYADLAR.length], r = rng(n);
...
test("tekrar yok: art arda aynı ad gelmez, son 8 atışta ad ve soyad tekrarlamaz", () => {
  for (const cins of ["k", "e"]) {
    const r = rng(cins === "k" ? 11 : 12), all = [];
    for (let i = 0; i < 4000; i++) all.push(A.rastgeleAd(cins, r, all.slice(-8)));
    for (let i = 1; i < all.length; i++) {
      assert.notEqual(all[i], all[i - 1], `art arda aynı ad: ${all[i]}`);
      const [a, s] = all[i].split(" "), prev = all.slice(Math.max(0, i - 8), i).flatMap(x => x.split(" "));
      assert.ok(!prev.includes(a) && !prev.includes(s), `son 8 atışta tekrar: ${all[i]} ← ${all.slice(Math.max(0, i - 8), i).join(", ")}`);
    }
  }
  // bozuk zar (hep aynı yüz) bile art arda aynı adı vermez
  const sabit = () => 0;
  let son = [];
  for (let i = 0; i < 20; i++) { const ad = A.rastgeleAd("k", sabit, son); assert.notEqual(ad, son.at(-1)); son = [...son, ad].slice(-8); }
});
