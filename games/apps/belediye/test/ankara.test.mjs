...
test("Ankara tavan yapınca oyun bitmez, genel merkez daveti gelir; taraflar çevrilmez", async () => {
  const X = await load(), r = rng(1), s = X.newGame(); s.month = 20;
  push(X, s, r);
...
test("ret: Ankara iner, makam sürer, manşet evrakı gelir; teklif her retle büyür", async () => {
  const X = await load(), r = rng(2), s = X.newGame(); s.month = 20;
  const seen = [];
  for (let k = 0; k < 4; k++) {
    push(X, s, r);
    const c = X.draw(s, r);
...
test("ikinci reddi Suat Bey sever (sıra ona kalır)", async () => {
  const X = await load(), r = rng(3), s = X.newGame(); s.month = 20; s.cnt.ankara_ret = 1;
...
test("kabul: terfi finali gelir, yenilgi gibi değil; oyun terfiyle biter", async () => {
  for (const [ret, key] of [[0, "a100_gm"], [1, "a100_mv"], [2, "a100"], [5, "a100"]]) {
    const X = await load(), r = rng(4 + ret), s = X.newGame(); s.month = 30; s.cnt.ankara_ret = ret;
    push(X, s, r); X.draw(s, r);
    X.choose(s, "R", r);
...
test("seçim evrakında Ankara tavanı sandığı bekletmez; davet seçimden sonra gelir", async () => {
  const X = await load(), r = rng(9), s = X.newGame();
...
test("davet ve ret evrakları içerik kurallarına uyar", async () => {
  const X = await load();
