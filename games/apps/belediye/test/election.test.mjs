...
test("Tekir nadiren aday olur, mama verilmişse daha sık", async () => {
  const rate = fed => {
    let n = 0;
    for (let g = 0; g < 4000; g++) {
      const r = rng(g + (fed ? 50000 : 40000)),
        s = E.newGame();
      if (fed) s.cnt.tekir = 3;
      const f = E.drawField(s, r);
      if (f.extras.includes("tekir")) n++;
    }
    return n / 4000;
  };
  const a = rate(false),
...
test("sonuç kartları: Tekir kazanırsa özel son, kaybedince kazananın adı, %50 altı zafer", async () => {
  ...
  const res = (cands, win) => ({
    ...
    margin: cands[0].pct - cands[1].pct,
  });
  const r1 = res(
    [
      { id: "tekir", pct: 41 },
  ...
  assert.equal(c3.kind, "sonuc");
  assert.match(c3.text, /3 adaylı/);
  assert.match(c3.text, /birinci birincidir/);
...
test("akış: her dönem seçimden önce adaylar ilan edilir, seçimde aynı liste yarışır", async () => {
  for (let g = 0; g < 60; g++) {
    ...
    for (let i = 0; i < 400 && !s.over; i++) {
      const c = E.draw(s, r);
      if (c.kind === "adaylar") {
        announced = { term: s.term, field: JSON.stringify(s.field) };
        assert.ok(s.month % E.TERM >= E.TERM - 10);
      }
      if (c.kind === "secim") {
...
test("erken seçim: esnaf tavan yapınca oyun bitmez, Hacı Bekir'li erken seçim gelir, döngüye girmez", async () => {
  ...
  assert.equal(s.pending?.type, "erken");
  const c = X.draw(s, r);
  assert.equal(c.kind, "secim");
  assert.ok(c.early);
  assert.match(c.text, /erken seçim/);
  assert.ok([s.earlyField.main, ...s.earlyField.extras].includes("bekir"), "Hacı Bekir aday değil");
  const term = s.term,
    eTerm = s.electionTerm;
  X.choose(s, "L", r);
...
test("erken seçim sonucu: kazanınca esnaf iner ve dönem sürer; Bekir'e kaybedince okey masası sonu", async () => {
  ...
  const term = s.term;
  X.choose(s, "L", rng(3));
  assert.ok(s.m.e <= 70, `esnaf ${s.m.e}`);
  assert.equal(s.term, term);
  assert.equal(s.over, null);
  const lostB = X.special(
    {
      type: "sonuc",
      oy: "30",
      win: false,
...
test("erken seçimde Hacı Bekir güçlüdür", async () => {
  ...
  for (let g = 0; g < 300; g++) {
    ...
    X.draw(s, r);
    const t = X.tally(s, r, true);
    sum += t.cands.find(c => c.id === "bekir").pct;
    n++;
  }
