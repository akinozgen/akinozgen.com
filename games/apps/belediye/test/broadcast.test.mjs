...
const FLAGS = [{}, { karaca: true, towers: true, devkavun: true, bisiklet: true, itfaiye_muze: true }, { itfaiye_satildi: true }];
// Gerçek tally() sonuçları + Tekir'in ve erken seçimin kazandığı elle kurulmuş sonuçlar
const RESULTS = (() => {
  const out = [];
  for (let g = 0; g < 300; g++) { const r = rng(g + 500), s = randomState(r); out.push(E.tally(s, r, g % 7 === 0)); }
  const base = out[0];
  const mk = (cands, early = false) => ({ ...base, cands, winner: cands[0].id, win: cands[0].id === "you", you: cands.find(c => c.id === "you").pct,
    margin: Math.round((cands[0].pct - cands[1].pct) * 10) / 10, early, order: cands.map(c => c.id).filter(id => id !== "you") });
  out.push(mk([{ id: "tekir", pct: 41 }, { id: "you", pct: 35 }, { id: "nermin", pct: 24 }]));
  out.push(mk([{ id: "bekir", pct: 44 }, { id: "you", pct: 38 }, { id: "nermin", pct: 18 }], true));
  out.push(mk([{ id: "you", pct: 50.3 }, { id: "nermin", pct: 49.7 }]));
  out.push(mk([{ id: "burak", pct: 30.1 }, { id: "you", pct: 29.9 }, { id: "kaan", pct: 20 }, { id: "albay", pct: 10 }, { id: "tuncay", pct: 10 }]));
  out.push(mk([{ id: "you", pct: 71 }, { id: "vekil", pct: 17 }, { id: "cengiz", pct: 7 }, { id: "muhtar", pct: 5 }]));
  return out;
})();
...
RESULTS.forEach((res, i) => {
  for (let k = 0; k < 6; k++) {
    const seed = i * 31 + k, r = rng(seed), ctx = ctxOf(res, i + k, r);
...
const ALL = [...OUT.kj.flatMap(x => [x.o.title, x.o.sub]), ...OUT.tick.flatMap(x => x.items.map(y => y.t)), ...OUT.quote.map(x => x.q), ...OUT.why,
  ...OUT.fx.flatMap(f => [f.ad, f.deger]), ...IDS.flatMap(id => [E.tvName(id, "Aslı Kavaklıoğlu"), E.tvShort(id, "Aslı Kavaklıoğlu")]), E.KANAL.ad];
...
test("adlar: her adayın adı ve kısa adı var, kısa adlar büyük harf ve birbirinden farklı", () => {
  for (const name of NAMES) {
    ...
    for (const [i, id] of IDS.entries()) {
      ...
      assert.ok(clean(shorts[i]) && shorts[i] === up(shorts[i]) && shorts[i].length <= 16, `${id} kısa adı: ${shorts[i]}`);
    }
...
test("alt bant: sayım başlığı açılan oranı Türkçe ekiyle yazar", () => {
  const res = RESULTS[0], at = f => { const s = new Set(); for (let g = 0; g < 80; g++) s.add(E.kj("sayim", { res, opened: f, playerName: "Ali" }, rng(g)).title); return [...s]; };
...
test("alt bant: sonuç evresinde kazanma, kaybetme, Tekir ve erken seçim ayrı ayrı anlatılır", () => {
  ...
  const wn = E.tvName(loss.winner), ws = E.tvShort(loss.winner);
  assert.ok(l.t.some(t => t.includes(up(wn)) || t.includes(ws) || /EL DEĞİŞTİRDİ|DEĞİŞİM/.test(t)), l.t.join(" | "));
  assert.ok(l.s.some(s => s.startsWith(wn + ": '")), "rakibin zafer sözü yok");
  assert.ok(k.t.every(t => /TEKİR|TÜYLÜ|MIRNAV/.test(t)), k.t.join(" | "));
...
test("alt bant: adaya özgü şakalar ve oyuncunun önde/geride olduğu satırlar gelir", () => {
  ...
  const subs = (leader, phase = "sayim", extra = {}) => { const s = new Set(); for (let g = 0; g < 120; g++) s.add(E.kj(phase, { res, playerName: "Ali", leader, pct: { [leader]: 40 }, opened: 0.4, ...extra }, rng(g)).sub); return [...s]; };
  ...
  assert.ok(subs(other).some(s => /^Ali geride|Ali cephesinden/.test(s)), "oyuncu geride satırı yok");
  assert.ok(subs(other, "lider", { prev: "you" }).some(s => s.includes("Ali liderliği kaptırdı") || s.includes("Ali geriye düştü")));
  // mahalleye özgü satır
  const bek = RESULTS.find(r => r.cands.some(c => c.id === "bekir"));
  const m = new Set(); for (let g = 0; g < 80; g++) m.add(E.kj("mahalle", { res: bek, mahalle: "Çarşı", playerName: "Ali" }, rng(g)).sub);
  assert.ok([...m].some(s => s.includes("Hacı Bekir")), "Çarşı'da Hacı Bekir anılmadı");
});

test("alt bant: ara evreler kesin sonucu ele vermez; lider bilinmiyorsa lidere dair satır gelmez", () => {
  for (const res of RESULTS.slice(0, 80)) for (let g = 0; g < 5; g++) for (const ph of ["acilis", "sayim", "mahalle", "son"]) {
    const o = E.kj(ph, { res, playerName: "Ali", opened: 0.4, mahalle: "Merkez" }, rng(g));
    assert.ok(!/(^|\P{L})(önde|öne geçti|geride|zirvede)(\P{L}|$)/iu.test(o.title + " " + o.sub), `${ph}: ${o.title} / ${o.sub}`);
  }
});

test("çeşitlilik: 50 tohumda sayım başlıkları ve alt satırları tekrara düşmez; seen verilince tekrar yok", () => {
  const res = RESULTS.find(r => r.cands.length >= 3), t = new Set(), s = new Set();
  for (let g = 0; g < 50; g++) {
    const r = rng(g * 7 + 1), ctx = { res, playerName: "Ali", ...running(res, 0.45, r), opened: 0.45 };
    const o = E.kj("sayim", ctx, r); t.add(o.title); s.add(o.sub);
  }
  assert.ok(t.size >= 5, `sayım başlığı çeşidi ${t.size}`);
  assert.ok(s.size >= 12, `sayım alt satırı çeşidi ${s.size}`);
  const seen = new Set(), r = rng(3), got = [];
  for (let i = 0; i < 12; i++) got.push(E.kj("sayim", { res, playerName: "Ali", leader: "you", pct: { you: 40 }, opened: 0.5, seen }, r).sub);
...
test("belirlenimcilik: aynı tohum aynı yayını üretir", () => {
  for (const [i, res] of RESULTS.slice(0, 40).entries()) {
    const run = seed => { const r = rng(seed), ctx = ctxOf(res, i, rng(seed + 1)); return JSON.stringify([PHASES.map(p => E.kj(p, ctx, r)), E.ticker(ctx, r), E.fx(i, r), E.winnerQuote(res.winner, ctx, r)]); };
...
test("son dakika: 8-12 haber, her biri ≤ 110, ilk haber seçimden, üç küme de her bantta var", () => {
  ...
  for (const { items } of OUT.tick) {
    ...
    for (const x of items) { assert.ok(clean(x.t) && x.t.length <= 110, `${x.t.length}: ${x.t}`); cats.add(x.cat); }
    assert.equal(items[0].cat, "secim");
    const k = items.filter(x => x.cat === "secim" || x.cat === "yerel").length, u = items.filter(x => x.cat === "ulusal").length, d = items.filter(x => x.cat === "dunya").length;
...
test("son dakika: havuzlar geniş, bazı dünya ve ülke haberleri Karakavak'a bağlanıyor", () => {
  const T = E.TV_TICK, n = k => T[k].length;
  assert.ok(n("secim") + n("yerel") + n("ulusal") + n("dunya") >= 100, "toplam haber");
  for (const k of ["ulusal", "dunya"]) {
    assert.ok(n(k) >= 25, `${k} ${n(k)}`);
    assert.ok(T[k].filter(t => /Karakavak|Fikret|Tekir/.test(t)).length >= 2, `${k} haberleri Karakavak'a hiç bağlanmıyor`);
  }
});
...
test("son dakika: adaylara, erken seçime, bayraklara ve sonuca göre değişir", () => {
  ...
  assert.doesNotMatch(texts(tvAll, res => ({ res, ...running(res, 0.6, rng(1)), opened: 0.6 })), /Kesin olmayan sonuçlara göre|yenilgiyi kabul|balkona çıktı/);
  assert.match(texts(tvAll, res => ({ res, opened: 1, playerName: "Ali" }), 120), /Kesin olmayan sonuçlara göre/);
  assert.match(texts(r => r.cands.some(c => c.id === "burak"), res => ({ res, opened: 0.2 })), /izleyici 12/);
});
function tvAll() { return true; }
...
test("döviz kutusu: Türkçe sayı biçimi, yön oku, i ile döner", () => {
  ...
  for (const n of ["KAVUN/TL", "ÇAY/BARDAK", "DOLAR", "EURO", "ALTIN/GR", "SİMİT", "TEKİR MAMASI/KG"]) assert.ok(names.has(n), n);
...
test("kazanan sözleri: her aday ve oyuncu için ≤ 70, en az üç farklı zafer ve üç farklı yenilgi sözü", () => {
  ...
  for (const id of IDS) {
    ...
    const won = { ...base, winner: id, win: id === "you", cands: [{ id, pct: 50 }, ...others.map(o => ({ id: o, pct: 25 }))] };
    const lost = { ...base, winner: others[0], win: others[0] === "you", cands: [{ id: others[0], pct: 50 }, { id, pct: 30 }, { id: others[1], pct: 20 }] };
    const w = new Set(), l = new Set();
    for (let g = 0; g < 80; g++) { w.add(E.winnerQuote(id, { res: won }, rng(g))); l.add(E.winnerQuote(id, { res: lost }, rng(g))); }
...
test("Neden? satırları: sayılar Türkçe, oy çalan adaylar adıyla anılır", () => {
  ...
  for (const t of OUT.why) assert.ok(clean(t) && t.length <= 120 && !/\d\.\d/.test(t.replace(/\d\.\d{3}|4\.0/g, "")), t);
  const res = RESULTS.find(r => (r.steal || []).some(x => x.v >= 1)), lines = E.whyLines(res, "Ali");
  for (const x of res.steal.filter(x => x.v >= 1)) assert.ok(lines.some(l => l.includes(E.tvName(x.id)) && l.includes(String(x.v).replace(".", ","))), x.id);
...
const BANNED = [
  ...
  "akp", "ak parti", "adalet ve kalkınma", "chp", "cumhuriyet halk", "mhp", "milliyetçi hareket", "hdp", "dem parti", "iyi parti",
  "saadet partisi", "deva partisi", "gelecek partisi", "zafer partisi", "yeniden refah", "refah partisi", "bbp", "dsp", "anap", "dyp",
  "cumhur ittifakı", "millet ittifakı",
  // siyasetçiler ve dünya liderleri
  "erdoğan", "kılıçdaroğlu", "imamoğlu", "bahçeli", "akşener", "babacan", "davutoğlu", "demirtaş", "erbakan", "türkeş", "ecevit",
  "demirel", "menderes", "özal", "çiller", "bakırhan", "dervişoğlu", "karamollaoğlu", "özdağ", "mansur yavaş", "özgür özel",
  "muharrem ince", "binali", "abdullah gül", "trump", "biden", "putin", "musk", "bezos", "zuckerberg", "macron", "merkel", "obama",
  "zelensky", "netanyahu", "xi jinping", "kim jong",
  // kurumlar
  "ysk", "yüksek seçim kurulu", "tedaş", "tüik", "sayıştay", "rtük", "diyanet", "tbmm", "meteoroloji genel",
  // kanallar ve gazeteler
  "trt", "cnn", "ntv", "habertürk", "show tv", "kanal d", "atv", "a haber", "halk tv", "sözcü", "tele1", "fox tv", "now tv",
  "star tv", "tv8", "beyaz tv", "kanal 7", "tv100", "bloomberg", "bbc", "hürriyet", "sabah gazetesi",
  // markalar ve kulüpler
  "coca-cola", "pepsi", "migros", "bim", "a101", "şok market", "turkcell", "vodafone", "türk telekom", "thy", "türk hava yolları",
  "trendyol", "hepsiburada", "yemeksepeti", "ülker", "arçelik", "vestel", "tesla", "spacex", "apple", "iphone", "google", "facebook",
  "instagram", "tiktok", "twitter", "whatsapp", "youtube", "netflix", "amazon", "microsoft", "openai", "chatgpt", "samsung",
  "bitcoin", "guinness", "eurovision", "galatasaray", "fenerbahçe", "beşiktaş", "trabzonspor", "thodex",
];
...
test("gerçek ad yok: parti, siyasetçi, kurum, kanal ve marka adları çıktıda ya da havuzlarda geçmez", () => {
  const pools = Object.values(E.TV_TICK).flat().map(e => (typeof e === "string" ? e : e[1]));
  const text = norm([...ALL, ...pools].join("\n"));
  for (const w of BANNED) {
    const re = new RegExp(`(^|[^\\p{L}\\p{N}])${norm(w).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}($|[^\\p{L}\\p{N}])`, "u");
